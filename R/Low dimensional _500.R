...
m1=list()
fx_eff_lmm=matrix(nrow = r,ncol=9)
ranef_lmm=list()
...
pred_50=matrix(nrow=r,ncol = n*round(p*m))
MSE9=matrix(nrow=r,ncol = 3)
colnames(MSE9)=c("Train","Test","Test2")

MSE10=matrix(nrow=r,ncol = 2)
colnames(MSE10)=c("Train","Test")
# Simulating data
set.seed(53216)
ID = rep(1:n, each = m)
...
pred_501=matrix(nrow=r,ncol = n*round(p*m))
NN1=matrix(ncol = 6, nrow = r)
...
for (i in 1:r) {
  ...
  e1 <- matrix(t(e), ncol = 1, byrow = TRUE)
  ...
  names(data)[names(data) == "X1.1"] <- "Intercept"
  test_X=subset(data[1:9],X3>round(p*m))
  train_X=subset(data[1:9],X3<=round(p*m))
  
  train_Y[[i]]<-subset(data$Y, X3<=round(p*m))
  test_Y[[i]]<-subset(data$Y, X3>round(p*m))
  
  sample = sample.split(data, SplitRatio = p)
  train2_X = subset(data[1:9], sample == TRUE)
  test2_X  = subset(data[1:9], sample == FALSE)
  ...
  datalist_train[[i]]=list(as.matrix(train_X[2:9]),
                           train_ID,
                           as.matrix(train_X[1]),
                           train_Y[[i]])
  ...
  datalist_test[[i]]=list(as.matrix(test_X[2:9]),
  ...
  datalist_test2=list(as.matrix(test2_X[2:9]),
  ...
  MSE4[i,1]=ME[[i]]$forest$mse[ME[[i]]$forest$ntree] #train MSE
  MSE4[i,2]=mean((predict_ME[i,]-test_Y[[i]])^2) #test MSE
  #MSE4[i,3]=mean((predict2_ME[i,]-test2_Y[[i]])^2)
  
  #Quantile regression LMM 
  qlmm[[i]]=lqmm(Y~1+X1+X2+X3+X4+Z_1+Z_2+Z_3+Z_4,~1,group=ID,
  ...
  qf[[i]]=quantregForest(x=train[[i]][1:8],y=train[[i]]$Y ,
                         data=train[[i]],
                         keep.inbag = T,mtry=5)
  # qf$rsq
  ...
  models <- list()
  for (q in quantiles) {
    dtrain <- xgb.DMatrix(data = train[[i]][,2:9], 
                          label = train[[i]]$Y)
    dtest <- xgb.DMatrix(data = test[[i]][,2:9], 
                         label = test[[i]]$Y)
    params <- list(
    ...
    models[[as.character(q)]] <- model
  }
  X_test= xgb.DMatrix(data = as.matrix(test[[i]][,2:9]))
  pred_50[i,]=predict(models[["0.5"]], newdata = X_test)
  
  MSE10[i,1]=mean((train_Y[[i]] - 
                      predict(models[["0.5"]], newdata = as.matrix(train[[i]][,2:9])))^2)
    
  
  MSE10[i,2]=mean((test_Y[[i]] - 
                     predict(models[["0.5"]], newdata = X_test))^2)
  ...
  pred_501[i,]=predict(models[["0.5"]], newdata = train[[i]][,2:9])
...
for (i in 1:r){
  for (j in 1:(round((1-p)*m)*n)){
    ...
    NN[i,6] =sum((pred_50[i,j]-test_Y[[i]][j])^2)
    
  }
}
NN=as.data.frame(NN)
colnames(NN)=c("LMM","REEM tree","MERF",
               "QLMM","QRF","Xgboostquantile")
MSE_results=cbind(mean(NN[,1]),mean(NN[,2]),mean(NN[,3]),
                  mean(NN[,4]),mean(NN[,5]),mean(NN[,6]))
fixed=matrix(ncol=(round((1-p)*m)*n),nrow=r)
...
for (i in 1:r){
  for (j in 1:(round((1-p)*m)*n)){
    ...
    fixed[i,5] =sum(((predict_qf[i,j])-
                    (train_Y[[i]][j]-e11[j]-u11[j]))^2)
    fixed[i,6] =sum(((pred_50[i,j])-
      (train_Y[[i]][j]-e11[j]-u11[j]))^2)
  }
}
fixedMSE_results=cbind(summary(fixed[,1]),summary(fixed[,2]),summary(fixed[,3]),
                   summary(fixed[,4]),summary(fixed[,5]),summary(fixed[,6]))

abss=matrix(ncol = 6, nrow = r)
for (i in 1:r){
  for (j in 1:(round((1-p)*m)*n)){
    ...
    abss[i,6] =sum(abs(pred_50[i,j]-test_Y[[i]][j]))
...
colnames(abss)=c("LMM","REEM tree","MERF","QLMM","QRF",
                 "Xgboostquantile")
MAE_results=cbind(mean(abss[,1]),mean(abss[,2]),
                  mean(abss[,3]),mean(abss[,4]),
                  mean(abss[,5]),mean(abss[,6]))
colnames(MAE_results)=c("LMM","REEM tree","MERF","QLMM","QRF",
                        "Xgboostquantile")
colnames(MSE_results)=c("LMM","REEM tree","MERF","QLMM","QRF",
                        "Xgboostquantile")
colnames(fixedMSE_results)=c("LMM","REEM tree","MERF","QLMM","QRF",
                         "Xgboostquantile")
write.csv(MSE_results,"scenario1rf_500_6_MSE_results.csv")
write.csv(fixedMSE_results,"scenario1rf_500_6_fixedMSE_results.csv")
write.csv(MAE_results,"scenario1rf_500_6_MAE_results.csv")
